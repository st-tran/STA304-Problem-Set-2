...
$$ \text{Prob}(h):= \begin{cases}1, &\text{if feelings\_life}\geq 5 \\ 0, &\text{otherwise}\end{cases} $$
...
```
For all of the categorical variables in Table 1, we use dummy variable coding with the variable at the top representing the baseline, in order to be able to assess what effect, if any, moving to a category would have compared to the baseline.
...
gss <- gss %>% mutate(happy=feelings_life>=5)
