...
- The continuous variables `age_at_first_birth`, `age` don't affect the model by much with odds-ratios slightly off from 1. $\beta_1$ is statistically significant but $\beta_2$ is not, with a p-value of 0.35, and so we fail to reject the null hypothesis that $\beta_2\neq 0$.
...
Using a variety of parameters with this model where each combination has a uniform probability of being chosen, we obtain the following histogram (Figure 11) that shows the class distribution of happiness across Canada. Note that the bins are lower-bound inclusive, explaining a number of predictions appearing beyond 1 (these probabilities are 1). The probabilities are very high, but that may be due to an imbalance in the survey data -- this will be discussed in the Weaknesses section of the report.
...
\textcolor{red}{needs to be cited}
